...
class Tela:
    def __init__(self, master):
        ...
        # Adiciona o bind da tecla Enter para a função de login
    ...
    def abrir_tela_cardapio(self, usuario_nome):
        ...
        # Salva a instância da tela para acessar o carrinho ao fechar
        self.tela_cardapio_instancia = TelaCardapio(
            top_cardapio, self.conn, self.cursor, usuario_nome, lambda: self.fechar_tela_cliente(top_cardapio))
    ...
    def fechar_tela_cliente(self, top_cardapio):
        # Antes de fechar, devolve o estoque dos itens do carrinho se houver algum
    ...
    def login(self, event=None):  # Adiciona 'event=None' para aceitar a chamada do bind
...
class TelaCardapio:
    def __init__(self, master, conn, cursor, usuario_nome, on_close_callback):
        ...
        ttk.Button(frm_topo, text="Ver Carrinho",
                   command=self.abrir_carrinho, bootstyle="info").pack(side=RIGHT)
    ...
    def carregar_produtos(self):
        for widget in self.scrollable_frame.winfo_children():
            widget.destroy()
        try:
            # Carrega todos, para mostrar esgotados
    ...
    def criar_bloco_produto(self, produto_data):
        ...
        btn_menos.pack()
        # Usando bind para passar o ID do produto para a função
    ...
    def adicionar_ao_carrinho(self, event, produto_id):
        ...
        if estoque_atual > 0:
            ...
            self.carregar_produtos()  # Recarrega para mostrar estoque atualizado
        else:
    ...
    def abrir_carrinho(self):
        ...
        TelaCarrinho(top_carrinho, self.conn, self.cursor,
                     self.carrinho, self.carregar_produtos)
...
class TelaCarrinho:
    def __init__(self, master, conn, cursor, carrinho_ref, refresh_cardapio_callback):
        ...
        self.conn = conn
        self.cursor = cursor
        self.carrinho = carrinho_ref  # Referência ao carrinho da tela de cardápio
    ...
    def atualizar_visualizacao_carrinho(self):
        ...
        if not self.carrinho:
            ...
            # Fecha a janela se o carrinho ficar vazio
